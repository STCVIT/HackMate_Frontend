      ...
      axios(`${url}/DN_Team/${teamid}`, {
        ...
        .then((response) => {
          ...
          if (team.team.hasOwnProperty('project_description')) {
            document.getElementById("projname").innerHTML = toTitleCase(team.team.project_name) + ".";
            ...
            document.project_form.personal_website.value = team.team.demonstration;
            document.getElementById("bio").innerHTML = team.team.project_description;
          }
          else {
          teamproj = localStorage.getItem("teamproj");
          document.getElementById("projname").innerHTML = toTitleCase(teamproj) + ".";
          }
...
function submitform() {
  ...
  firebase.auth().currentUser.getIdToken().then((id) => {
    ...
    if (flag == 0) {
      axios
        .patch(`${url}/DN_Team/update/${teamid}`,
          {
        ...
        .then((response) => {
          ...
          swal("SUCCESS!!", "Project has been created successfully", "success").then(okay => {
            // if (okay) {
